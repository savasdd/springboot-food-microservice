...
public class StockGrpcService extends StockServiceGrpc.StockServiceImplBase {
    ...
    public void getStock(StockRequest request, StreamObserver<StockResponse> responseObserver) {
        try {
            ...
            responseObserver.onNext(!stock.isEmpty() ? StockResponse.newBuilder().setStockId(stock.get(0).getId()).setAvailableItems(stock.get(0).getAvailableItems()).setStatus(200).build() : StockResponse.newBuilder().setStatus(400).build());
