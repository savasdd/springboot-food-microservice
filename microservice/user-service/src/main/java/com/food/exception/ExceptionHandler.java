...
public class ExceptionHandler {
    ...
    public ResponseEntity<ExceptionResponse> jpaSystemException(Exception ex) {
        log.error("JpaSystemException", ex);
        ExceptionResponse error = new ExceptionResponse();
        error.errorMessage = getLangMessage(ex.getCause().getMessage(), null);
        return new ResponseEntity<>(error, HttpStatus.BAD_REQUEST);
    }

    @org.springframework.web.bind.annotation.ExceptionHandler(PersistenceException.class)
    public ResponseEntity<ExceptionResponse> persistenceException(PersistenceException ex) {
        log.error("PersistenceException", ex);
        ExceptionResponse error = new ExceptionResponse();
        error.errorMessage = getLangMessage("{review.data}", null);
        return new ResponseEntity<>(error, HttpStatus.BAD_REQUEST);
    }

    @org.springframework.web.bind.annotation.ExceptionHandler(DataIntegrityViolationException.class)
    public ResponseEntity<ExceptionResponse> dataIntegrityViolationException(DataIntegrityViolationException ex) {
        log.error("DataIntegrityViolationException", ex);
        ExceptionResponse error = new ExceptionResponse();
        error.errorMessage = getLangMessage("{before.data.enter}", null);
        return new ResponseEntity<>(error, HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<ExceptionResponse> transactionSystemException(TransactionSystemException ex) {
        ...
        if (t instanceof ConstraintViolationException) {
            error.errorMessage = StringUtils.join(getViolationsMsg((ConstraintViolationException) t), ", ");
        } else if (t.getCause() != null) {
            Throwable tt = t.getCause();
            if (tt instanceof ConstraintViolationException) {
                error.errorMessage = StringUtils.join(getViolationsMsg((ConstraintViolationException) tt), ", ");
            } else {
