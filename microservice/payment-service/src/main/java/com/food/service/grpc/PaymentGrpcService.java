...
public class PaymentGrpcService extends PaymentServiceGrpc.PaymentServiceImplBase {
    ...
    public void getPayment(PaymentRequest request, StreamObserver<PaymentResponse> responseObserver) {
        try {
            ...
            responseObserver.onNext(!payment.isEmpty() ? PaymentResponse.newBuilder().setPaymentId(payment.get(0).getPaymentId().toString()).setAmountAvailable(payment.get(0).getAmountAvailable().doubleValue()).setStatus(200).build() : PaymentResponse.newBuilder().setStatus(400).build());
